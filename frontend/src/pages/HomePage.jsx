...
export default function HomePage({ onStart }) {
  ...
  return (
    <div className="min-h-screen bg-[var(--bg-page)] text-[var(--text-secondary)] selection:bg-sky-500/30 font-sans overflow-x-hidden transition-colors duration-200">
      
      {/* Navbar Minimalist */}
      <nav className="absolute top-0 w-full z-50 transition-all">
      ...
      <main className="relative z-10 pt-32 md:pt-48 pb-20">
        <section className="max-w-[1000px] mx-auto px-6 text-center">
          <h1 className="text-5xl md:text-7xl lg:text-[80px] font-semibold tracking-tight mb-8 leading-[1.1] text-[var(--text-heading)]">
            Data Analyst AI yang <span className="text-glow-blue">memukau</span>
          </h1>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4 mb-24">
            <button 
              onClick={onStart}
              className="h-12 flex items-center justify-center px-8 rounded-md bg-sky-600 text-white font-medium text-base hover:bg-sky-500 transition-colors shadow-lg shadow-sky-600/20"
            >
              Mulai Analisis
            </button>
            <button 
              onClick={onStart}
              className="h-12 flex items-center justify-center px-8 rounded-md border border-[var(--border-primary)] bg-transparent text-[var(--text-heading)] font-medium text-base hover:bg-[var(--bg-hover)] transition-colors"
            >
              Lihat Demo
            </button>
          </div>
          ...
          <div className="relative w-full max-w-4xl mx-auto h-[200px] flex justify-between items-start">
            {/* The nodes */}
            <div className="relative z-10 flex justify-between w-full px-12 md:px-24 text-sm font-mono text-slate-400">
              <div className="bg-[var(--bg-page)] px-4 py-1.5 rounded-full border border-sky-500/30 text-sky-500 z-10">Upload</div>
              <div className="bg-[var(--bg-page)] px-4 py-1.5 rounded-full border border-sky-500/30 text-sky-500 z-10">Analisis</div>
              <div className="bg-[var(--bg-page)] px-4 py-1.5 rounded-full border border-sky-500/30 text-sky-500 z-10">Visualisasi</div>
              <div className="bg-[var(--bg-page)] px-4 py-1.5 rounded-full border border-sky-500/30 text-sky-500 z-10">Prediksi</div>
            </div>
            ...
            <svg className="absolute inset-0 w-full h-[300px] pointer-events-none" viewBox="0 0 1000 300" preserveAspectRatio="none">
              ...
              <g className="branch-path" stroke="url(#line-glow)" strokeWidth="1.5" fill="none" filter="url(#glow-blur)">
                {/* Node 1 to Center */}
                <path d="M 150 15 C 150 120, 500 80, 500 300" />
                {/* Node 2 to Center */}
                <path d="M 383 15 C 383 120, 500 80, 500 300" />
                {/* Node 3 to Center */}
                <path d="M 616 15 C 616 120, 500 80, 500 300" />
                {/* Node 4 to Center */}
                <path d="M 850 15 C 850 120, 500 80, 500 300" />
              </g>
      ...
      <div className="h-px w-full bg-gradient-to-r from-transparent via-[var(--border-primary)] to-transparent"></div>
      ...
      <section id="features" className="py-32 relative">
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="flex flex-col lg:flex-row items-center gap-16 lg:gap-24">
              ...
              <RevealSection>
                ...
                <ul className="space-y-4 mb-10 text-[var(--text-secondary)]">
                  <li className="flex items-start gap-3">
                    ...
                    <span>Pro Mode dengan Planner Agent & Executor Agent (LangGraph)</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-sky-500 shrink-0"></span>
                    <span>Laporan yang berlimpah: PDF Render, Jupyter Notebook, & Chart Interaktif</span>
                  </li>
                  <li className="flex items-start gap-3">
                    <span className="mt-1.5 w-1.5 h-1.5 rounded-full bg-sky-500 shrink-0"></span>
                    <span>Asynchronous background job yang tahan terhadap refresh halaman</span>
                  </li>
                </ul>
            ...
            <div className="flex-1 w-full bg-[var(--bg-tertiary)] rounded-2xl border border-[var(--border-primary)] p-2 shadow-2xl relative overflow-hidden">
               {/* Header mock */}
                 ...
                 <div className="flex-1 bg-[#050812] p-5 font-mono text-xs flex flex-col relative">
                    ...
                    <div className="bg-[#0f172a] p-3 rounded border border-slate-800 text-slate-300 flex-1 whitespace-pre-wrap overflow-hidden">
                      ...
                      <div className="mt-3 w-full h-16 bg-slate-800/50 rounded flex items-end p-2 gap-1 opacity-80">
                         <div className="w-4 h-[40%] bg-sky-500/60 rounded-t-sm"></div>
                         <div className="w-4 h-[70%] bg-sky-500/60 rounded-t-sm"></div>
                         <div className="w-4 h-[30%] bg-sky-500/60 rounded-t-sm"></div>
                         <div className="w-4 h-[90%] bg-sky-500/60 rounded-t-sm"></div>
                      </div>
                    </div>
                 </div>
               </div>
      ...
      <section id="demo" className="py-24 border-t border-[var(--border-primary)] bg-[var(--bg-secondary)]">
        <div className="max-w-[1400px] mx-auto px-6">
          ...
          <div className="relative">
            ...
            <div className="hidden md:block absolute top-[20%] left-[10%] right-[10%] h-px bg-gradient-to-r from-sky-900/0 via-sky-500/30 to-sky-900/0" />
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-8 relative z-10">
              {[
                ...
                <RevealSection key={i} delay={`reveal-delay-${i}`} className="bg-[var(--bg-card)] border border-[var(--border-primary)] p-8 rounded-2xl flex flex-col items-center text-center hover:border-sky-500/30 transition-colors shadow-[var(--shadow-lg)]">
        ...
        <div className="max-w-[1400px] mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <RevealSection key={i} delay={`reveal-delay-${(i%3)+1}`}>
                <div className="card-hover-glow bg-[var(--bg-card)] border border-[var(--border-primary)] rounded-xl p-6 h-full flex flex-col items-start text-left cursor-default shadow-[var(--shadow-sm)]">
